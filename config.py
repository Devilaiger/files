...
# Fuzzy search match threshold (0-100).
# 60 is a good default — catches partial names and minor typos without too many
# false positives. Raise to 80+ for stricter matching, lower to 50 for more lenient.
# NOTE: Phase 2 (substring match) runs before fuzzy, so exact substrings always
# match regardless of this threshold.
FUZZY_THRESHOLD: float = float(os.getenv("FUZZY_THRESHOLD", "60"))
...
# Storage channel — bot must be admin here.
# ALL trigger response messages are COPIED here (not forwarded — no origin header).
